...
void Sys_Init(void)
{
    ...
    // timer0_pwm_config();
    {
        ...
        T0DATA = 255; // 100%占空比（确保大于TxLOAD的值就可以）
        PWM0EC = 0;   // 禁止PWM输出
    ...
    // key_config();
    {
        ...
        P11OE = 0; // 输入模式
        /*
            之前的io配置，默认将io配置为输出模式，输出低电平，
            会在低功耗唤醒之后观察到有一段低电平出现，不影响程序功能
        */
    }
...
void key_event_handle(void)
{
    ...
    sub_mode_status = SUB_MODE_NONE; // 每次触发有效的按键事件，清空该标志位，不让定时器处理相关操作

    if (FLAG_IS_DEVICE_OPEN)
    {
        // 如果设备正在运行
        if (KEY_EVENT_MODE_PRESS == key_event || KEY_EVENT_MODE_HOLD == key_event)
        {
            if (MODE_1 == mode_flag)
            {
                // 下面的操作放到定时器+状态机来处理
                ...
                // PWM0EC = 1;
                sub_mode_status = SUB_MODE_1_BEGIN;
                mode_flag = MODE_2;
            }
    ...
    for (i = 0; i < 10; i++)      //  // 每55ms进入一次，循环内每次间隔约4.8ms
    {
        ...
        if (FLAG_IS_IN_CHARGING)
        {
            ...
            if (charge_time_cnt >= ((u32)3 * 60 * 60 * 1000 + (u32)7 * 60 * 1000) && 0 == flag_is_led_breath_disable) // 3h7min
            {
                ...
                LED_RED_OFF();  // 关闭充电时的呼吸灯
                LED_GREEN_ON(); // 点亮绿灯，表示充满电
            }
...
void turn_dir_scan_handle(void)
{
    if (FLAG_IS_DEVICE_OPEN)
    {
        ...
        //     flag_ctl_dir)
        if (turn_dir_ms_cnt >= ((u32)60 * 1000))
        {
            if (PWM1EC) // PWM1 正在输出
            {
                PWM1EC = 0;
...
void low_power_scan_handle(void)
{
    ...
    if (FLAG_DURING_CHARGING_BAT_IS_NULL || // 只插着充电器且没有电池时，不进入低功耗
        ...
        (1 == P11D))                        // 如果检测到 开关/模式 按键按下，不进入低功耗(交给按键事件处理函数来判断是否要开机)
    {
        return;
    }
...
label:
    ...
    if (adc_val < ADCDETECT_CHARING_THRESHOLD && P11D == 0)
    {
        // 如果没有按下开机按键、没有插入充电器
        // 关闭ADC，继续进入休眠：
        goto label;
    ...
    while (1)
    {
        ...
        if (FLAG_IS_IN_CHARGING)
        {
            ...
            */
            ...
            {
                u16 i;
                for (i = 0; i < 260; i++) // 会在800mA以上，800mA附近±100mA
                {
                    if (tmp_bat_val > 2)
                    {
                        tmp_bat_val--;
    ...
    {
        {
            ...
            if (__1ms_cnt >= 10)
            {
...

                {
                    ...
                    if (sub_mode_status == SUB_MODE_1_BEGIN)
                    {
                        cnt = 0;
                        HEATING_ON();
                        ...
                        if (PWM1EC)
                        {
                            // 如果pwm1正在输出，关闭它
                            PWM1EC = 0;
                            flag_is_enable_pwm0_in_isr = 1; // 一旦定时结束，打开 pwm0
                        }
                        else
                        {
                            // 如果pwm0正在输出，关闭它
                            PWM0EC = 0;
                            flag_is_enable_pwm0_in_isr = 0; // 一旦定时结束，打开 pwm1
                        }

                        sub_mode_status = SUB_MODE_1_HANDLING;
                    }
                    else if (sub_mode_status == SUB_MODE_1_HANDLING)
                    {
                        cnt++;
                        if (cnt >= 1000)
                        {
                            cnt = 0;
                            if (flag_is_enable_pwm0_in_isr)
                            {
                                PWM0EC = 1;
                            }
                            else
                            ...
                            sub_mode_status = SUB_MODE_1_END;
                        }
                    }
                    else
                    {
                        cnt = 0;
                        flag_is_enable_pwm0_in_isr = 0;
                    }
                }
